...
def dooray_webhook():
    ...
    if command in callback_ids:
        ...
        if member_roles:
            ...
            logger.warning("⚠️ 멘션 포맷 아님 또는 파싱 실패, fallback 처리 시도")

            if not input_text:
                fallback_name = get_member_name_by_id(userId)
                if fallback_name:
                    assignee_text = f"@{fallback_name}"
                    logger.info("✅ userId 기반 이름으로 담당자 설정: %s", assignee_text)
                else:
                    assignee_text = ""
                    logger.warning("⚠️ userId=%s 로부터 이름을 찾지 못해 assignee_text를 비워둡니다.", userId)
            else:
